...
export function Toolbar({
  ...
  onOpenRoot, onSearch, onSave, onSetViewMode, onUndo, onRedo, onSetLang,
}: ToolbarProps) {
  ...
  return (
    <div className="fs-toolbar">
      ...
      {/* 语言开关：右上角（用户原话点名的位置）——toolbar 是一整行 flex 布局，两个
          ...
          子节点天然贴到行的最右端。
          本轮（UI 双语）只切界面文案这一半，两根线留给下一轮接，别当成漏做了：
          1) 点击不调用 `spec/setLang`——那会把语言写进 front-matter，是另一份状态；
          2) 初始值不取自 `OpenResult.lang`——这个字段现在已经存在（core 侧另一轮加的），
             但按 brief 的既定范围本轮仍不接这根线，留给下一轮把"载入工作区后开关自动
             对齐契约里写的语言"一起接上。
